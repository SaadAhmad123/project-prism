    ...
    <div id="model-range-component" class="px-4 lg:px-16">
        ...
        <div class="flex flex-wrap justify-between">
            <div class="pr-4 sm:pr-8 w-full md:w-1/2">
                <input placeholder="Enter an observation" :max="this.config.validConfig.max" :min="this.config.validConfig.min" :step="this.config.validConfig.step" :type="config.validConfig.inputType" v-model="input" class="w-full shadow appearance-none border rounded py-2 px-3 text-gray-700 hover:border-blue-5 mb-6" />
            ...
            <div class="w-full md:w-1/2">
                <div class="flex flex-wrap items-start justify-end">
                    <model-alert-component class="mb-4" :warning="isNaN(parseInt(input))" text="Please enter a valid value"/>
                    <model-display-component v-if="config.displayPoints && !isNaN(parseInt(input))" :title="config.displayName" :text="parseFloat(input).toFixed(1)"/>
                    <model-display-component v-if="config.displayPoints && !isNaN(parseInt(input))" title="Points" :text="parseFloat(points).toFixed(1)"/>
                </div>
            </div>
        </div>
    </div>
...
export default {
   ...
   computed : {
       ...
       'points' : function(){return this.config.modelFunction(this.input)},
